...
{
    //下面给channel设置相应的回调函数，poller通知channel其感兴趣的事件已发生，channel回调相应的函数
...
{
    //TcpConnection本身没有new什么资源，成员变量的指针也都是智能指针，所以不用做相应的资源释放操作
...
void TcpConnection::send(const std::string &buf)
{
    ...
    {
        if(loop_->isInLoopThread()) //当前的loop是否在对应的线程中
        {
...
//发送数据，应用写的快，内核发送数据慢，需要把待发送数据写入缓冲区，并且设置水位回调
void  TcpConnection::sendInLoop(const void* data, size_t len)
{
    ...
    if(!channel_->isWriting() && outputBuffer_.readableBytes() == 0)
    {
        ...
        if(nwrote >= 0) //发送成功
        {
            ...
            {
                //数据全部发送完成，不必再给channel设置epollout事件，不用再执行handleWrite()
                loop_->queueInLoop(
        ...
        {
            nwrote = 0;
            if(errno != EWOULDBLOCK) //EWOULDBLOCK -> 非阻塞，因为没有数据，正常返回
            {
    ...
    if(!faultError && remaining > 0) //没有发生错误，并且缓冲区还有数据没有发送
    {
        //目前发送缓冲区剩余的待发送数据的长度
...
void TcpConnection::handleRead(Timestamp receiveTime)
{
    ...
    if(n > 0)
    {
        ...
        //shared_from_this()返回当前TcpConnection对象的智能指针
